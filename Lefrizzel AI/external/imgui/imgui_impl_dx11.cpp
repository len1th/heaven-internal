    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                auto IsValidSRV = [](void* ptr) -> bool {
                    if (!ptr || (uintptr_t)ptr < 0x10000 || (uintptr_t)ptr > 0x7FFFFFFFFFFFull)
                        return false;
                    __try {
                        void* vtbl = *reinterpret_cast<void**>(ptr);
                        if (!vtbl || (uintptr_t)vtbl < 0x10000 || (uintptr_t)vtbl > 0x7FFFFFFFFFFFull)
                            return false;
                        void* fn = *reinterpret_cast<void**>(vtbl);
                        return (fn && (uintptr_t)fn >= 0x10000 && (uintptr_t)fn <= 0x7FFFFFFFFFFFull);
                    } __except (1) {
                        return false;
                    }
                };

                if (!IsValidSRV(texture_srv)) {
                    ImGuiIO& io = ImGui::GetIO();
                    ImTextureID fontTex = io.Fonts ? io.Fonts->TexRef.GetTexID() : ImTextureID_Invalid;
                    texture_srv = (ID3D11ShaderResourceView*)(intptr_t)fontTex;
                }
